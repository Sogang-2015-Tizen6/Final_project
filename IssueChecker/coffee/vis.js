...
Bubbles = function() {
  ...
  chart = function(selection) {
    return selection.each(function(rawData) {
      ...
      .attr("id", "bg")
      .append("image")
      .attr("xlink:href", "http://www.kccosd.org/files/testing_image.jpg");
      
      
      //.attr("xlink:href", "http://cspro.sogang.ac.kr/~cse20121581/int/img_url.php?" + d.name);

   svg.append("rect")
      .attr("fill", "url(#bg)");
      <defs>
		<pattern id="image" patternUnits="userSpaceOnUse" height="100" width="100">
			<image x="0" y="0" height="100" width="100"	xlink:href="http://www.kccosd.org/files/testing_image.jpg"></image>
			</pattern>
			</defs>
      ///////////////*/
  ...
  updateNodes = function() {
    ...
    $.ajax({
  		type: 'GET',
  		url: 'http://cspro.sogang.ac.kr/~cse20121581/int/img_url.php?' + kk.name,
  		success: function(result) {
  			$("#ok").append("image").attr("id",kk.name).attr("x","0").attr("y","0").attr("height","100").attr("width","100").attr("xlink:href",result);
  			
  		}
	});
    */
    /*
	$.get("http://cspro.sogang.ac.kr/~cse20121581/int/img_url.php?q=" + idValue(d), function(data) {
		alert(" " + data);
		//$("#bub_img").attr("xlink:href",data);
	});.attr("fill","url(#t)")*/
  ...
  updateLabels = function() {
    ...
    label.style("font-size", function(d) {
      return Math.max(1, rScale(rValue(d) / 8)) + "px";
...
*/
  return d3.json("http://cspro.sogang.ac.kr/~cse20121581/int/prac3.php?q=kor", display);
});
